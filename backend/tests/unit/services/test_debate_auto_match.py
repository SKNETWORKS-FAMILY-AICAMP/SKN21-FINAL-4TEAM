...
class TestDebateAutoMatcher:
    ...
    async def test_no_platform_agents_publishes_timeout(self):
        ...
        with patch(
            "app.services.debate.matching_service.publish_queue_event", new_callable=AsyncMock
        ) as mock_pub:
    ...
    async def test_match_created_and_event_published(self):
        ...
        with (
            patch("app.services.debate.matching_service.DebateMatch") as MockMatch,
            patch(
                "app.services.debate.matching_service.publish_queue_event", new_callable=AsyncMock
            ) as mock_pub,
            patch("app.services.debate.matching_service.asyncio.create_task"),
        ):
    ...
    def test_start_stop(self):
        ...
        with patch("app.services.debate.matching_service.asyncio.create_task"):
            matcher.start()
