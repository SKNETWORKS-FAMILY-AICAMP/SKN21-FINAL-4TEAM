...
def upgrade() -> None:
    ...
    op.add_column("debate_match_queue", sa.Column("expires_at", sa.DateTime(timezone=True), nullable=True))
    # 기존 행 backfill: joined_at + 120초
    op.execute("UPDATE debate_match_queue SET expires_at = joined_at + INTERVAL '120 seconds'")
    # NOT NULL 제약 추가
