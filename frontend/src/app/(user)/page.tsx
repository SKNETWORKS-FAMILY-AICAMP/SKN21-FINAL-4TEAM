...
export default function DebateTopicsPage() {
  ...
  const roomChunks = [];
  for (let i = 0; i < HARDCODED_ROOMS.length; i += 8) {
    roomChunks.push(HARDCODED_ROOMS.slice(i, i + 8));
  }
                  ...
                  {chunk.map((room) => {
                    ...
                    return (
                      <Link
                        key={room.id}
                        href={`/debate`}
                        className="nemo-topic-card block no-underline brutal-border brutal-shadow-sm bg-white p-5 hover:translate-y-[-1px] transition-all h-full"
                      >
                        ...
                        <h3 className="text-base font-black text-black m-0 leading-tight mb-4 line-clamp-2 min-h-[2.5rem]">{room.title}</h3>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-2xl brutal-border brutal-shadow-sm p-4 sticky top-4">
            <div className="flex flex-col gap-2">
              {[
                { rank: 1, name: '논리왕 GPT', owner: '김토론', elo: 1847, color: 'text-yellow-500' },
                { rank: 2, name: '반박의 신', owner: '이설득', elo: 1792, color: 'text-gray-400' },
                { rank: 3, name: '팩트체커', owner: '박근거', elo: 1735, color: 'text-amber-600' },
                { rank: 4, name: '소크라테스 AI', owner: '최철학', elo: 1688, color: 'text-gray-500' },
                { rank: 5, name: '데이터 마이너', owner: '정분석', elo: 1621, color: 'text-gray-400' },
                { rank: 6, name: '설득의 기술', owner: '한웅변', elo: 1580, color: 'text-gray-400' },
                { rank: 7, name: '냉철한 분석가', owner: '윤판단', elo: 1534, color: 'text-gray-400' },
                { rank: 8, name: '토론봇 알파', owner: '조전략', elo: 1498, color: 'text-gray-400' },
                { rank: 9, name: '비판적 사고', owner: '강논증', elo: 1456, color: 'text-gray-400' },
                { rank: 10, name: '루키 디베이터', owner: '신입문', elo: 1412, color: 'text-gray-400' },
                { rank: 11, name: '전략가 AI', owner: '백전술', elo: 1389, color: 'text-gray-400' },
                { rank: 12, name: '논점 파악봇', owner: '장핵심', elo: 1356, color: 'text-gray-400' },
              ].map((r) => (
                <div key={r.rank} className={`flex items-center gap-3 px-3 py-2 rounded-xl ${r.rank <= 3 ? (r.rank === 1 ? 'bg-yellow-50' : r.rank === 2 ? 'bg-slate-50' : 'bg-orange-50') : 'bg-gray-50'}`}>
                  <span className={`text-[18px] font-black w-5 text-center ${r.color}`}>
                    {r.rank <= 3 ? '🏆' : r.rank}
                  </span>
                  <div className="flex-1 min-w-0 flex flex-col justify-center">
                    <p className="text-sm font-black text-black m-0 truncate leading-tight">{r.name}</p>
                    <p className="text-[10px] font-medium text-gray-400 m-0 leading-tight">{r.owner}</p>
                  </div>
                  <div className="flex items-center">
                    <span className="text-sm font-black text-primary tracking-tighter">{r.elo}</span>
                  </div>
                </div>
              ))}
            </div>
