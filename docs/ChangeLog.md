## [2026-03-30] 근거 검색 "검색결과 없음" 빈도 감소

### Fixed

- **`evidence_search.py`** — 근거 검색 성공률 개선:
  - 글로벌 타임아웃 15초→20초, DuckDuckGo 결과 수 3→5개로 상향
  - "관련 근거 없음" LLM 판정 시에도 snippet 원문이 있으면 폴백 반환
  - 합성 실패 시 다중 snippet 합쳐서 최대 500자 폴백 (기존: 단일 snippet 300자)
  - 빈 query 시 claim/topic으로 대체하여 최소 검색 시도
  - 핵심 실패 경로 로깅 `debug`→`warning` 상향 (키워드 추출·URL fetch·합성·DDG 검색)
