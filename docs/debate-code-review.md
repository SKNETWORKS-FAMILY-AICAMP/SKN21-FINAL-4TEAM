...
토론 로직은 `backend/app/services/` 아래 5개 파일로 분리되어 있다.

```
services/
├── debate_matching_service.py  — 큐 등록 + DebateAutoMatcher + ready_up()
├── debate_engine.py            — 턴 루프 진입점, 벌점 감지, 판정 후처리 오케스트레이션
├── debate_orchestrator.py      — LLM 호출 3종 (review_turn / judge / calculate_elo)
├── debate_broadcast.py         — Redis Pub/Sub SSE 발행
...
debate_matching_service.py
    └─ ready_up() → asyncio.create_task(run_match())
                                        │
                              debate_engine.py
                                  ├─ _run_turn_loop()
                                  ...
                                  └─ _finalize_match()
                                        ...
                                        ├─ agent_service.update_elo()
                                        ├─ season_service.update_season_stats()
                                        └─ promotion_service.record_match_result()
```
...
**구현 위치:** `debate_matching_service.py` — `DebateMatchingService.join_queue()`
...
```python
# debate_matching_service.py — ready_up()
...
**구현 위치:** `debate_matching_service.py` — `DebateAutoMatcher`
...
```python
# debate_engine.py — _execute_match()
if has_local:
...
**구현 위치:** `debate_engine.py` — `validate_response_schema()`, `detect_repetition()`
...
```python
# debate_engine.py — validate_response_schema()
...
```python
# debate_engine.py — detect_repetition()
...
**구현 위치:** `debate_orchestrator.py` — `DebateOrchestrator.review_turn()`
...
```python
# debate_engine.py — _run_turn_loop() parallel 모드
...
**구현 위치:** `debate_orchestrator.py` — `DebateOrchestrator._judge_with_model()`
...
```python
# debate_orchestrator.py — _judge_with_model()
...
**구현 위치:** `debate_orchestrator.py` — `calculate_elo()` / `debate_engine.py` — `_finalize_match()`

표준 ELO에 **점수차 배수**를 추가한 변형 공식을 사용한다.

```python
# debate_orchestrator.py — calculate_elo()
...
```python
# debate_engine.py — _finalize_match() 처리 순서
...
**구현 위치:** `debate_promotion_service.py` — `DebatePromotionService`
...
```python
# debate_promotion_service.py — check_and_trigger()
...
```python
# debate_promotion_service.py — record_match_result()
...
```python
# debate_engine.py — _finalize_match()
if res == "draw":
...
```python
# debate_orchestrator.py — _review_fallback()
...
```python
# debate_orchestrator.py — review_turn()
try:
...
| 벌점을 두 단계로 분리 (규칙 + LLM) | `debate_engine.py` + `debate_orchestrator.py` | 명백한 위반은 코드로 빠르게, 의미론적 위반은 LLM으로 정확하게 |
