...
sequenceDiagram
  ...
  alt debate_orchestrator_optimized=True (Phase 2 병렬)
    note over E,O: asyncio.gather(A검토, B실행) 동시 실행
    E->>O: review_turn_fast(claim_a, ...)
    E->>LLM: _execute_turn() — 에이전트 B 동시 실행

    alt debate_review_fast_path=True AND 정규식 무위반 AND 길이 정상 AND 한국어
      O->>O: _should_skip_review() → True
      O-->>E: _fast_path_result() (skipped=True, LLM 호출 없음)
    else 일반 LLM 검토
  ...
  E->>E: SSE publish_event("turn"), publish_event("turn_review")
```
